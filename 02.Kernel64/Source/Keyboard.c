...
/****************************************************
    키보드 컨트롤러와 키보드 디바이스를 제어하는 함수들
*****************************************************/
// 키보드 컨트롤러 출력버퍼(port 0x60)에 수신된 데이터 존재 여부 판단
BOOL kIsOutputBufferFull( void )
{
    // 상태 레지스터(포트 0x64)에서 읽은 값에 출력 버퍼 상태 비트(비트 0)가 1로 설정되어 있으면 출력 버퍼에 키보드가 전송한 데이터가 존재함
    if( kInPortByte( 0x64 ) & 0x01 ) // 어셈블리어 명령인 in 호출
    {
        return TRUE;
    }
    return FALSE;
...
// 키보드 컨트롤러 입력 버퍼(포트 0x60)에 프로세서가 쓴 데이터가 남아있는지 여부 판단
BOOL kIsInputBufferFull( void )
{
    // 상태 레지스터(포트 0x64)에서 읽은 값에 입력 버퍼 상태 비트(비트 1)가 1로 설정되어 있으면 아직 키보드가 데이터를 가져가지 않았음
    if( kInPortByte( 0x64 ) & 0x02 ) // 어셈블리어 명령인 in 호출
    {
        return TRUE;
    }
    return FALSE;
...
BOOL kActivateKeyboard( void )
{
    ...
    // 키보드 컨트롤러 컨트롤 레지스터(포트 0x64)에 키보드 활성화 커맨드(0xAE)를 전달하여 키보드 디바이스 활성화
    kOutPortByte( 0x64, 0xAE ); // 어셈블리어 명령인 out 호출
    ...
    kOutPortByte( 0x60, 0xF4 ); // 어셈블리어 명령인 out 호출
    
    // 키보드 디바이스로부터 ACK가 올 때까지 대기
    // ACK가 오기 전에 키보드 출력 버퍼(포트 0x60)에 키 데이터가 저장되어 있을 수 있으므로
    // 키보드에서 전달된 데이터를 최대 100번 반복 확인하며 이후에 올 ACK를 확인
    for( j = 0 ; j < 100 ; j++ )
    {
        // 충분한 시간(0xFFFF 루프를 수행한) 이후에도 출력 버퍼(포트 0x60)가 차 있지 않으면 무시하고 읽음
        for( i = 0 ; i < 0xFFFF ; i++ )
        {
        ...
        // ACK(0xFA)가 오면 키보드 활성화 성공
        if( kInPortByte( 0x60 ) == 0xFA ) // 어셈블리어 명령인 in 호출
        {
            return TRUE;
        }
    }
...
BYTE kGetKeyboardScanCode( void )
{
    ...
    return kInPortByte( 0x60 ); // 어셈블리어 명령인 in 호출
}
...
BOOL kChangeKeyboardLED( BOOL bCapsLockOn, BOOL bNumLockOn, BOOL bScrollLockOn )
{
    ...
    kOutPortByte( 0x60, 0xED ); // 어셈블리어 명령인 out 호출

    // 입력 버퍼 확인 for 키보드가 커맨드를 가져갔는지 확인
    for( i = 0 ; i < 0xFFFF ; i++ )
    {
    ...
    for( j = 0 ; j < 100 ; j++ )
    {
        ...
        if( kInPortByte( 0x60 ) == 0xFA ) // ACK(0xFA), 어셈블리어 명령인 in 호출
        {
            break;
        }
    }
    ...
    kOutPortByte( 0x60, ( bCapsLockOn << 2 ) | ( bNumLockOn << 1 ) | bScrollLockOn ); // 어셈블리어 명령인 out 호출
    for( i = 0 ; i < 0xFFFF ; i++ )
    {
...
// A20 게이트를 활성화(입출력 포트는 키보드 컨트롤러의 설정값으로 이를 변경한다.)
void kEnableA20Gate( void )
{
    ...
    kOutPortByte( 0x64, 0xD0 ); // 키보드 컨트롤러가 출력 포트값을 출력 버퍼에 복사해준다, 어셈블리어 명령인 out 호출
    
    // 출력 버퍼 확인 for 출력 포트 값 읽기
    for( i = 0 ; i < 0xFFFF ; i++ )
    {
        // 출력 버퍼(포트 0x60)가 차있으면 데이터를 읽을 수 있음 
        if( kIsOutputBufferFull() == TRUE )
    ...
    // 출력 버퍼(데이터를 임시로 저장하는 버퍼)에 복사된 출력 포트 값(키보드 컨트롤러의 설정값)을 읽음
    ...
    kOutPortByte( 0x64, 0xD1 ); // 키보드 컨트롤러가 "다음으로 입력되는" 입력 버퍼의 값을 출력 포트값으로 복사해준다, 어셈블리어 명령인 out 호출
    
    // 입력 버퍼(0x60)에 A20 게이트 비트가 1로 설정된 값을 전달
    kOutPortByte( 0x60, bOutputPortData ); // 어셈블리어 명령인 out 호출
}
...
void kReboot( void )
{
    ...
    kOutPortByte( 0x60, 0x00 ); // 이값이 키보드 컨트롤러의 입력 포트로 복사된다, 어셈블리어 명령인 out 호출
...
{
    // 숫자 패드나 확장 키 범위를 제외한 범위(스캔 코드 2~53)에서 영문자가 아니면 숫자 또는 기호
...
BOOL kIsUseCombinedCode( BYTE bScanCode ) // bScanCode 는 스캔코드이므로 BYTE 가 맞다.
{
    BYTE bDownScanCode;
...
void UpdateCombinationKeyStatusAndLED( BYTE bScanCode )
{
    ...
    if( bScanCode & 0x80 ) // AND 1000_0000 == 최상위 비트값이 1인 경우
    {
        bDown = FALSE;
        bDownScanCode = bScanCode & 0x7F; // AND 0111_1111 == 최상위 비트값이 0인 경우
    }
    else // 0이면 눌림을 의미
    {
        bDown = TRUE;
    ...
    if( ( bDownScanCode == 42 ) || ( bDownScanCode == 54 ) ) // 왼쪽 Shift 키의 스캔 코드(42) 또는 오른쪽 Shift 키의 스캔 코드(54)
    {
