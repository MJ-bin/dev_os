...
int main(int argc, char *argv[]) {
    ...
    //1. 부트 로더 파일을 열어서 모든 내용을 디스크 이미지 파일로 복사
    ...
    //2. 32비트 커널 파일을 열어서 모든 내용을 디스크 이미지 파일로 복사
    ...
    //3. 64비트 커널 파일을 열어서 모든 내용을 디스크 이미지 파일로 복사
    ...
    // 파일 크기를 섹터 크기인 512바이트로 맞추기 위해 나머지 부분을 0x00으로 채움
    iKernel64SectorCount = AdjustInSectorSize(iTargetFd, iSourceSize); // 32비트 커널의 전체 사이즈 반환(섹터수)
    printf("[INFO] %s size = [%d] and sector count = [%d]\n", argv[2], iSourceSize, iKernel32SectorCount);


    //4. 디스크 이미지에 커널 정보를 갱신
...
void WriteKernelInformation(int iTargetFd, int iTotalKernelSectorCount, int iKernel32SectorCount) {
    ...
    */
    ...
    // 부트 로더를 제외한 "커널 총 섹터 수" && "보호 모드 커널 섹터 수" 저장
