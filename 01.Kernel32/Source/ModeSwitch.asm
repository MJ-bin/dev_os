...
; 1. 64비트를 지원하는 CPU인지 확인: CPUID를 반환
...
kReadCPUID:
    ...
    push eax             ; 함수에서 임시로 사용하는 레지스터를 함수의 마지막 부분에서
...
; 2. IA-32e 모드로 전환하고 64비트 커널을 수행
; PARAM: 없음
kSwitchAndExecute64bitKernel:
; 2.1 CR4 컨트롤 레지스터의 PAE 비트를 1로 설정: 36비트 이상의 주소(64GB)에 접근가능
    mov eax, cr4                ; CR4 컨트롤 레지스터의 값을 EAX 레지스터에 저장
    or eax, 0x20                ; PAE 비트(비트 5)를 1로 설정
    mov cr4, eax                ; PAE 비트가 1로 설정된 값을 CR4 컨트롤 레지스터에 저장

; 2.2 CR3 컨트롤 레지스터에 PML4 테이블의 Base Address 저장: 페이징을 위한 설정
    mov eax, 0x100000           ; EAX 레지스터에 PML4 테이블이 존재하는 0x100000(1MB)을 저장
    mov cr3, eax                ; CR3 컨트롤 레지스터에 0x100000(1MB)를 저장

; 2.3 IA32_EFER.LME 필드를 1로 설정: IA-32e 모드를 활성화
...
; 2.4 CR0 컨트롤 레지스터 NW 비트(비트 29) = 0, CD 비트(비트 30) = 0, PG 비트(비트 31) = 1로 설정: 캐시 기능과 페이징 기능을 활성화
