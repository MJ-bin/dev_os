...
void kInitializePageTables( void )
{
    ...
    for( i = 0 ; i < PAGE_MAXENTRYCOUNT * 64 ; i++ )
    {
        ...
        kSetPageEntryData( 
                            &( pstPDEntry[ i ] ), // 처리할 구조체 포인터
                            ( i * ( PAGE_DEFAULTSIZE >> 20 ) ) >> 12, // 상위 8비트 어드레스(엔트리에서는 32~39 비트), 사실상 i >> 11 과 동일한 연산
                            dwMappingAddress, // 하위 11비트 어드레스(엔트리에서는 21~31 비트)
                            PAGE_FLAGS_DEFAULT | PAGE_FLAGS_PS, // 하위 속성 플래그(엔트리에서는 0~20 비트)
                            0 // 상위 32~63비트 속성 플래그(엔트리에서는 40~63 비트)
                        );
